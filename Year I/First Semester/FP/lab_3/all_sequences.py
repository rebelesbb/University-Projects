#5,6 + documentatie cazuri testare

import re

def print_menu():
    """
    Prints out the menu
    """
    print("1.Citire lista")
    print("2.Determinare secvente de lungime maxima care au toate elementele egale")
    print("3.Determinare secvente de lungime maxima care au toate elementele distincte intre ele")
    print("4.Iesire din aplicatie\n")

def read_list() :
    """
     Reads a list of numbers
     returns a list of integer numbers
    """
    initial_list = input("Introduceti lista de numere: ")
    initial_list = initial_list.strip(', ')
    initial_list = re.split(r", | ", initial_list)
    list = [int(number) for number in initial_list]
    return list


def determine_longest_sequence_equal_elements(list):

    list_length = len(list)

    longest_sequence_start = longest_sequence_end = 0

    all_longest_sequences = []

    for current_sequence_start in range(list_length) :

        current_sequence_end = current_sequence_start + 1

        while current_sequence_end < list_length and list[current_sequence_start] == list[current_sequence_end] :
            current_sequence_end += 1

        current_sequence_end -= 1

        if longest_sequence_end - longest_sequence_start + 1 < current_sequence_end - current_sequence_start + 1 :
            longest_sequence_end = current_sequence_end
            longest_sequence_start = current_sequence_start

            all_longest_sequences.clear()
            all_longest_sequences.append( list[current_sequence_start:current_sequence_end + 1] )

        elif longest_sequence_end - longest_sequence_start + 1 == current_sequence_end - current_sequence_start + 1 :
            all_longest_sequences.append( list[current_sequence_start:current_sequence_end + 1] )

        current_sequence_start = current_sequence_end

    if longest_sequence_end - longest_sequence_start + 1 < current_sequence_end - current_sequence_start + 1 :
        longest_sequence_end = current_sequence_end
        longest_sequence_start = current_sequence_start

        all_longest_sequences.clear()
        all_longest_sequences.append(list[current_sequence_start:current_sequence_end + 1])

    elif longest_sequence_end - longest_sequence_start + 1 == current_sequence_end - current_sequence_start + 1:
        all_longest_sequences.append(list[current_sequence_start : current_sequence_end + 1])

    if len(all_longest_sequences) == 1 :
        print("Secventa de lungime maxima care are toate elementele egale este:")
    else :
        print("Secventele de lungime maxima care au toate elementele egale sunt:")

    for element in all_longest_sequences :
        print(element)

def determine_longest_sequence_unequal_elements(list) :

    """
    Determines and prints the longest sequence of consecutive elements,
    which all have distinct values from one another
    parameter list is a list of integer numbers
    """

    list_length = len(list)

    #the indices of the first and last element of the longest sequence found in the list
    longest_sequence_start = longest_sequence_end = 0

    #in case there is more than one sequence of greatest length, we use this list to store all of them
    all_longest_sequences = []

    for current_sequence_start in range(list_length) :

        current_sequence_end = current_sequence_start + 1

        while current_sequence_end < list_length and len(list[current_sequence_start : current_sequence_end + 1]) == len(set(list[current_sequence_start : current_sequence_end + 1])):
            current_sequence_end += 1

        current_sequence_end -= 1

        #checks if the current list has a length greater than the longest one found so far
        if longest_sequence_end - longest_sequence_start + 1 < current_sequence_end - current_sequence_start + 1:
            longest_sequence_end = current_sequence_end
            longest_sequence_start = current_sequence_start

            #clears the list of the previously found sequences, and add the newly found longest sequence in the list
            all_longest_sequences.clear()
            all_longest_sequences.append(list[current_sequence_start: current_sequence_end + 1])

        elif longest_sequence_end - longest_sequence_start + 1 == current_sequence_end - current_sequence_start + 1:

            #if it has the same length as the previously found sequence, we add it to the list,
            all_longest_sequences.append(list[current_sequence_start: current_sequence_end + 1])

        current_sequence_start = current_sequence_end

    #checks if the last sequence in the list satisfies the condition, because it wasn't checked in the for loop
    if longest_sequence_end - longest_sequence_start + 1 < current_sequence_end - current_sequence_start + 1 :
        longest_sequence_end = current_sequence_end
        longest_sequence_start = current_sequence_start

        all_longest_sequences.clear()
        all_longest_sequences.append(list[current_sequence_start : current_sequence_end + 1])

    elif longest_sequence_end - longest_sequence_start + 1 == current_sequence_end - current_sequence_start + 1 :
        all_longest_sequences.append(list[current_sequence_start : current_sequence_end + 1])

    #prints the sequences
    if len(all_longest_sequences) == 1 :
        print("Secventa de lungime maxima care are toate elementele distincte intre ele este:")
    else :
        print("Secventele de lungime maxima care au toate elementele distincte intre ele sunt:")

    for element in all_longest_sequences :
        print(element)

    print('\n')

list = []
option = 1
print_menu()
while True:
    option = int(input("Introduceti optiunea pe care o doriti: "))
    if option == 1 :
        list = read_list()
        print('Lista introdusa este:\n', list, '\n')
    elif option == 2 :
        determine_longest_sequence_equal_elements(list)
    elif option == 3 :
        determine_longest_sequence_unequal_elements(list)
    elif option == 4 :
        break