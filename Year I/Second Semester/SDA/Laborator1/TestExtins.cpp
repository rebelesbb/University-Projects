...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	...
	for (unsigned int i = 0; i < elemente.size(); i++) { //scoatem pe rand elemente din vectorul dat de iterator, verificam sa fie in colectie si le stergem si din vector si din c4. Incepem stergerea cu ultimul
        ...
        //cout<< lastElem;
        //cout << c4.cauta(lastElem)<< '\n';
        //cout<< c4.nrAparitii(lastElem)<<'\n';
		assert(c4.cauta(lastElem) == true);
		c4.sterge(lastElem);

        //assert(number - 1 == c4.dim());
	}
    ...
    Colectie c1;
    IteratorColectie ic_invalid = c1.iterator();
...
void testTransformaInMultime()
{
    ...
    assert(c.transformaInMultime() == 0);
    for(index = 0; index < 20; index++)
        assert(c.nrAparitii(index) == 1);

    for(index = 10; index < 20; index++) {
        for (int i = 1; i <= 5; i++)
            c.adauga(index);
        assert(c.nrAparitii(index) == 6);
    }

    assert(c.transformaInMultime() == 5 * 10);
    ...
    assert(c.transformaInMultime() == 5 * 20);
}
