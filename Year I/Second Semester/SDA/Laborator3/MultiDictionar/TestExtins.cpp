...
void testAdaugaInexistente()
{
	...
	for (int i = 1; i <= 10; i++)
		dictDeAdaugat.adauga(i, 2 * i);

	assert(m1.dim() == 0);

	int adaugate = m1.adaugaInexistente(dictDeAdaugat);
	assert(adaugate == 10);
	assert(m1.dim() == 10);
	...
	adaugate = m2.adaugaInexistente(dictDeAdaugat);
	assert(adaugate == 0);
	assert(m2.dim() == 10);
	...
	adaugate = m3.adaugaInexistente(dictDeAdaugat);
	assert(adaugate == 10);
	assert(m3.dim() == 20);
	...
	for (int i = 1; i <= 10; i++)
		if (i % 2)
			m4.adauga(i, i);
		else m4.adauga(i, 2 * i);

	assert(m4.dim() == 10);

	adaugate = m4.adaugaInexistente(dictDeAdaugat);
	assert(adaugate == 5);
	assert(m4.dim() == 15);

	MD dictDeAdaugat2;
	for (int i = 1; i <= 10; i++)
		dictDeAdaugat2.adauga(1, 1);

	assert(dictDeAdaugat2.dim() == 10);

	/// Best Case
	MD m5;
	m5.adauga(1, 1);

	adaugate = m5.adaugaInexistente(dictDeAdaugat2);
	assert(adaugate == 0);
